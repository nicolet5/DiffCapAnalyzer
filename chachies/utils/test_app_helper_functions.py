...
test_datatype = 'ARBIN'
test_filename_mac = 'test_data_mac.csv'
...
def test_generate_model(): 
	...
	feedback = generate_model(df_clean, test_filename, 
							  peak_thresh, test_db)
	assert type(feedback) == type(html.Div([]))
	names_list = get_table_names(test_db)
	filename_pref = test_filename.split('.')[0]
...
def test_generate_model_for_maccor(): 
	...
	feedback = generate_model(df_clean, test_filename_mac, 
							  peak_thresh, test_db)
	assert type(feedback) == type(html.Div([]))
	names_list = get_table_names(test_db)
	filename_pref = test_filename_mac.split('.')[0]
	list_new_tables =  ['-ModPoints', 'ModParams', '-descriptors']
	for i in list_new_tables: 
		assert filename_pref + i in names_list
	os.remove(test_db)
