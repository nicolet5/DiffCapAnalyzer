...
app.layout = html.Div([
	...
	html.Div([
		...
		html.A('Download Peak Descriptors CSV', download = "descriptors.csv", id = 'my-link-1'),
		html.Br(),
		html.A('Download Cleaned Cycles CSV', download = "cleaned_cycles.csv", id = 'my-link-2'),
		html.Br(),
		html.A('Download Model Points CSV', download = "model_points.csv", id = 'my-link-3'),
		],
		style={
			'width': '98%',
			'margin': '10px'
...
def update_figure2(filename, peak_thresh, n_clicks, show_gauss, desc_to_plot, cd_to_plot, peaknum_to_plot):
	...
	peak_vals_df = get_file_from_database(filename.split('.')[0] + '-descriptors',database_sel)
...
def update_link_1(value):
	if value is not None: 
		peak_vals_df = get_file_from_database(value.split('.')[0] + '-descriptors', database)
		if peak_vals_df is not None: 
...
def update_link_2(value):
	if value is not None: 
		clean_set_df = get_file_from_database(value.split('.')[0] + 'CleanSet', database)
...
def update_link_3(value):
	if value is not None: 
		mod_points_df = get_file_from_database(value.split('.')[0] + 'ModPoints', database)
