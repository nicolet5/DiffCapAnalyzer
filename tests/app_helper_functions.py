...
def parse_contents(decoded, filename, datatype, database, auth, windowlength = 9, polyorder = 3):
	...
	core_filename = dbq.get_filename_pref(filename)
	cleanset_name = core_filename + 'CleanSet'
	...
	if ans_p == True: 
		...
		new_peak_thresh = 0.7 
		feedback = generate_model(df_clean, core_filename, new_peak_thresh, database)
		return 'That file exists in the database: ' + str(core_filename)
	else:
		...
		try:
			...
			new_peak_thresh = 0.7
			feedback = generate_model(df_clean, core_filename, new_peak_thresh, database)
			return 'New file has been processed: ' + str(core_filename)
		except Exception: 
...
def generate_model(df_clean, core_filename, peak_thresh, database):
	...
	dbw.dbfs.update_database_newtable(mod_pointsdf, core_filename + '-ModPoints', database)
	# this will replace the data table in there if it exists already 
	dbw.dbfs.update_database_newtable(param_df, core_filename + 'ModParams', database)
	...
	dbw.param_dicts_to_df(core_filename + 'ModParams', database)		
