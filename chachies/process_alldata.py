...
# for each in c_list:
# 	process_one_file(each, c_database)	
# # then put them all in a df here and label as cs2
df_c = pd.DataFrame()
for each in c_list:
...
df_c['CS2-1-K16-0'] = 1
# writer = pd.ExcelWriter('c_descriptors.xlsx')
# df_c.to_excel(writer,'Sheet1')
# writer.save()
# then put k in a new database and put them in a df here and label as k 
# then combine into one df


# for each in k_list:
	# process_one_file(each, k_database)	
# then put them all in a df here and label as cs2
df_k = pd.DataFrame()
for each in k_list:
	descript_name = each.split('.')[0] + 'ModParams-descriptors'
...
df_k['CS2-1-K16-0'] = 0
# writer2 = pd.ExcelWriter('k_descriptors.xlsx')
# df_k.to_excel(writer2,'Sheet1')
# writer2.save()
...
# for each in k_list:
# 		process_one_file(each, database)
# # 	except (Exception):
# 		print(str(each) + 'was not processed')
