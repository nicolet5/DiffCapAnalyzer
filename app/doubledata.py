...
def update_figure1(selected_step,contents,filename,date,selected_row_indices):
    if contents is not None:
        ...
        charge, discharge = ccf.sep_char_dis(data)
        rows_df = charge
        filtered_data = rows_df[rows_df['Cycle_Index'] == selected_step]
...
def update_figure2(selected_step2,contents,filename,date,selected_row_indices2):
    if contents is not None:
        data = parse_contents(contents, filename, date)
        charge, discharge = ccf.sep_char_dis(data)
        rows_df = discharge
        rows_df = parse_contents(contents, filename, date)
